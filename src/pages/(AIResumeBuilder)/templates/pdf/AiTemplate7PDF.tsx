    ...
    <View style={{ marginTop: 3 }}>
      {lines.map((line, i) => (
        <View key={i} style={{ flexDirection: 'row', marginTop: i > 0 ? 1.5 : 0 }}>
          <Text style={{ width: 10, flexShrink: 0, fontSize: 9, color: '#4b5563', textAlign: 'justify' }}>{line.startsWith('•') ? '•' : ''}</Text>
          <Text style={{ flex: 1, fontSize: 9, color: '#4b5563', lineHeight: 1.5, textAlign: 'justify' }}>{line.startsWith('•') ? line.substring(1).trim() : line}</Text>
        </View>
      ))}
    </View>
  );
      ...
      <Page size="A4" style={{ paddingTop: 36, paddingBottom: 28, paddingLeft: 44, paddingRight: 44, fontSize: 9, fontFamily: 'Helvetica' }}>
        ...
        {personal.aboutCareerObjective && (
          <>
            ...
            <Text style={{ fontSize: 9.5, color: '#374151', lineHeight: 1.6, textAlign: 'justify' }}>{htmlToPlain(personal.aboutCareerObjective)}</Text>
          </>
        )}
        ...
        {education.higherEducation.filter(e => e.enabled).reverse().map((edu: any, i) => (
          <View key={i} style={{ marginBottom: 6 }}>
            ...
            {edu.result ? <Text style={{ fontSize: 9, color: '#6b7280' }}>{fmtResult(edu.result, edu.resultFormat)}</Text> : null}
          </View>
        ))}
        {education.preUniversityEnabled && education.preUniversity?.instituteName && (
          <View style={{ marginBottom: 6 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <Text style={{ fontSize: 8.5, color: '#9ca3af' }}>{fmtYear(education.preUniversity.yearOfPassing)}</Text>
            </View>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ fontSize: 9, color: '#6b7280' }}>{education.preUniversity.instituteName}</Text>
              <Text style={{ fontSize: 9, color: '#6b7280' }}>{education.preUniversity.boardType}</Text>
            </View>
            {education.preUniversity.result ? <Text style={{ fontSize: 9, color: '#6b7280' }}>{fmtResult(education.preUniversity.result, education.preUniversity.resultFormat)}</Text> : null}
          </View>
        )}
        {education.sslcEnabled && education.sslc?.instituteName && (
          <View style={{ marginBottom: 6 }}>
            ...
            {education.sslc.result ? <Text style={{ fontSize: 9, color: '#6b7280' }}>{fmtResult(education.sslc.result, education.sslc.resultFormat)}</Text> : null}
          </View>
        )}
